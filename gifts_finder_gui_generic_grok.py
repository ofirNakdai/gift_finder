...
class GiftFinderApp:
    ...
    def show_loading_indicator(self, parent):
        ...
        self.loading_progress.place(relx=0.5, rely=0.5, anchor="center")
        self.loading_progress.start(10)  # Start animation
        parent.update()
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root, text="Gift Finder", font=("Segoe UI", 20, "bold"), bg="#F5F7FA", fg="#343A40").pack(pady=20)
        ...
        load_button.bind("<Enter>", lambda e: load_button.config(bg="#218838"))
        load_button.bind("<Leave>", lambda e: load_button.config(bg="#28A745"))

        self.file_label = tk.Label(self.root, text="לא נבחר קובץ", fg="#6C757D", bg="#F5F7FA", font=("Segoe UI", 10))
        self.file_label.pack(pady=5)

        self.search_frame = tk.Frame(self.root, bg="#F5F7FA")
        self.search_frame.pack(pady=15)

        tk.Label(self.search_frame, text="🔍 חפש לפי שם:", bg="#F5F7FA", fg="#343A40", font=("Segoe UI", 12)).pack(side=tk.LEFT, padx=5)
        ...
        search_button.bind("<Enter>", lambda e: search_button.config(bg="#0056b3"))
        search_button.bind("<Leave>", lambda e: search_button.config(bg="#007BFF"))

        show_button = tk.Button(
            self.root,
            text="📑 הצג את תוכן הקובץ",
            command=self.show_full_df,
            bg="#6C757D",
            fg="white",
            font=("Segoe UI", 11),
            padx=12,
        ...
        show_button.bind("<Enter>", lambda e: show_button.config(bg="#5a6268"))
        show_button.bind("<Leave>", lambda e: show_button.config(bg="#6C757D"))

        self.canvas = tk.Canvas(self.root, borderwidth=0, background="#F5F7FA", highlightthickness=0)
        self.results_frame = tk.Frame(self.canvas, background="#F5F7FA")
    ...
    def create_result_card(self, row, card_index):
        ...
        for i, value in enumerate(row):
            font = ("Segoe UI", 11, "bold") if i == 0 else ("Segoe UI", 11)
            label = tk.Label(
                card,
    ...
    def load_file(self):
        ...
        try:
            ...
            choice_popup.configure(bg="#F5F7FA")
            ...
            tk.Label(
                choice_popup,
                text="בחר את הכותרות עבור הקובץ:",
            ...
            if is_first_row_valid:
                ...
                tk.Label(
                    choice_popup,
                    text="השורה הראשונה אינה תקינה ככותרות (חסרים ערכים או לא טקסט)",
                    fg="#DC3545",
                    bg="#F5F7FA",
                    font=("Segoe UI", 10)
                ).pack(pady=8)
            ...
            ttk.Radiobutton(
                choice_popup,
                text="השתמש בכותרות ברירת המחדל",
                variable=header_choice,
                value="default"
            ).pack(anchor="e", pady=5)
            if is_first_row_valid:
                ttk.Radiobutton(
                    choice_popup,
                    text="השתמש בשורה הראשונה ככותרות",
                    variable=header_choice,
                    value="first_row"
                ).pack(anchor="e", pady=5)
            ttk.Radiobutton(
                choice_popup,
                text="אף אחת מהכותרות לא נכונה (השתמש בשמות עמודות ברירת מחדל)",
                variable=header_choice,
                value="no_headers"
            ).pack(anchor="e", pady=5)
            ...
            def confirm_choice():
                try:
                    ...
                    self.df.fillna("ערך ריק", inplace=True)
            ...
            confirm_button.bind("<Enter>", lambda e: confirm_button.config(bg="#218838"))
            confirm_button.bind("<Leave>", lambda e: confirm_button.config(bg="#28A745"))
    ...
    def show_full_df(self):
        ...
        container = tk.Frame(popup, bg="#F5F7FA")
